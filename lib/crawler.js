...
module.exports.crawl = function(user, provider, callback) {
  ...
  if(config.recrawl) delete user[provider].last_update;
  crawlers[provider].sync(user[provider], function(err, datas) {
    if(datas.last_update) last_update = datas.last_update;
    datas = datas.data;
    if(err) {
...
function insert(user, provider, type, data, callback) {
  ...
  console.log('saving', data.length, 'objects of type', type + '/' + provider, 'for user', user._id);
